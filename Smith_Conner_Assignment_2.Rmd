...
ventura_cost_tax/162

mojave_cost_tax/93

*(4)* Tax revenue generated is $`r tax_revenue` based on 500AF of total use.
...
*(2)* Total cost of this approach is $`r total_cost_trade` (same as tax).
This is $`r (total_cost_cap - total_cost_trade)` lower than the cost of the cap with no trade.
...
*(4)* No tax revenue is generated from this policy. Instead, some districts may pay others to increase abatement more so they can use more water. We did not calculate this, although it can be assumed that the per unit price of AF for the trade value is the same as the tax amount, which is $`r tax_level`. It is presumed that districts with lower costs under the cap and trade scenario (Mojave and Antelope) would compensate districts with higher costs (Kern and Ventura) to incentivize them to abate more.
